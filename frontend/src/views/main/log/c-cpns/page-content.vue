...
// 6.监听logStore.pageList的更新，重新渲染table
logStore.$onAction(({name, after}) => {
  after(() => {
...
const {pageList, pageTotalCount} = storeToRefs(logStore)
...
function fetchPageListData(formData: any = {}) {
  ...
  logStore.postPageListAction(props.contentConfig.pageName, data, params)
}
...
const handleDeleteBtnClick = (id: number) => {
  ...
  ElMessageBox.confirm("确定要删除吗？", "提示", {
    ...
    logStore.deletePageByIdAction(props.contentConfig.pageName, id)
  }).catch(() => {
...
const handleStatusChange = async (itemData: any) => {
  console.log('status change', itemData)
  console.log("itemData.id: ", itemData.id)
  const {success, error} = await logStore.editPageDataAction(props.contentConfig.pageName, itemData.id, itemData)
  if (success) {
...
const batchDelete = async () => {
  try {
    ...
    const deletePromises = selectedFiles.value.map(file =>
        logStore.deletePageByIdAction(props.contentConfig.pageName, file.id)
    )
        ...
        >
          <template v-for="item in contentConfig?.propsList" :key="item.prop">
            ...
            <template v-else-if="item.type === 'handler'">
              <el-table-column v-if="isUpdate || isDelete || isCopy || isView" align="center" v-bind="item">
                <template #default="scope">
