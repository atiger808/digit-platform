...
const modalConfigRef = computed(() => {
  ...
  const devices = vpnStore.listDevices.map(item => {
    return { label: `${item.device_name}（${item.device_number}）` , value: item.id }
  })

  const vpn_devices = vpnStore.listVpnDevices.map(item => {
    return { label: `${item.device_name}（${item.device_number}）`, value: item.id }
  })
  ...
  modalConfig.formItems.forEach(item => {
    ...
    } else if (item.prop === 'vpn_device_id') {
      item.options = []
      item.options.push(...vpn_devices)
    }
  })
