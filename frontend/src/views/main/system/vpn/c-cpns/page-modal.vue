    ...
    if (valid) {
      ...
      if (!isNewRef.value && editData.value) {
        ...
        const {
          success,
          error
        } = await vpnStore.editPageDataAction(props.modalConfig.pageName, editData.value.id, infoData)
        if (success) {
        ...
        // 新增
        console.log('新增部门')
        const {success, error} = await vpnStore.addPageDataAction(props.modalConfig.pageName, infoData)
        if (success) {
...
// 生成随机密码
const generateRandomPassword = (prop: any) => {
  const length = 16;
  ...
  formData[prop] = password;
  ElMessage.success('密码生成成功，请复制保存');
}
    ...
    >
      ...
      <template #header="{ titleId, titleClass }">
        <div class="dialog-header">
          ...
          <div class="stat-desc"><span style="color: red; justify-content: space-between ; align-items: center">*</span>
            开头为必填项，其他项可选填
          </div>
          ...
          <template v-for="(item, index) in modalConfig.formItems" :key="index">
            <el-form-item :label="item.label" :prop="item.prop">
              ...
              <template v-else-if="item.type === 'password'">
                ...
                <!--                  </el-col>-->
                <!--                  <el-col :span="4">-->
                <!--                    <el-button v-if="item.type === 'password'"-->
                <!--                               @click="generateRandomPassword(item.prop)"-->
                <!--                               icon="Refresh"-->
                <!--                               title="生成随机密码"-->
                <!--                    >-->
                <!--                    </el-button>-->
