  ...
  <div class="login-container">
    ...
    <div class="box-item login">
      ...
      <a-form ref="formRef" class="login-form" :model="loginForm" :rules="rules">
        ...
        <a-form-item name="captchaCode" v-if="captchaCodeShowFlag">
          <a-input class="captcha-input" v-model:value.trim="loginForm.captchaCode" placeholder="请输入验证码"/>
          <img class="captcha-img" :src="captchaBase64Image" @click="getCaptcha" title="点击刷新"/>
        </a-form-item>
        ...
        <a-form-item>
          ...
          <span style="float: right"> <router-link to="/register">没有账号？去注册</router-link></span>
        </a-form-item>
        <a-form-item v-if="sliderVerify">
      ...
      <div class="copyright" style="justify-content: space-between; align-items: center; margin: auto; margin-top: 10px">
        <a href="https://beian.miit.gov.cn" target="_blank" style="text-decoration: none; color: #2c3e50; font-size: small; padding: 10px 0;">浙ICP备2025196633号-1</a>
      </div>
...
import loginGif from '@/assets/images/login/left-bg2.png';
...
import {type ElForm, ElMessage, ElLoading} from "element-plus";
...
const sliderVerify = ref(false)
// 事件处理函数
const onCaptchaSuccess = () => {
  console.log('验证成功')
  sliderVerify.value = true
}

const onCaptchaFail = () => {
  console.log('验证失败')
  sliderVerify.value = false
}
...
async function onLogin() {
  ...
  formRef.value.validate().then(async () => {
    try {
      ...
      let {success, error, attempts} = await loginStore.loginAccountAction(data)

      if (!success) {
        if (attempts) {
          getCaptcha()
          captchaCodeShowFlag.value = true
        }
        message.error(error)
        return
      }
      captchaCodeShowFlag.value = false;
...
//--------------------- 验证码 ---------------------------------
const captchaCodeShowFlag = ref(false);
